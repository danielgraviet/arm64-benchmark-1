...
def test_settings_defaults():
    knobs = rlp_client_tuning.settings()
    assert knobs["http_max_connections"] == 512
    assert knobs["wait_poll_start_s"] == 0.25
    assert knobs["wait_poll_factor"] == 1.5
    assert knobs["wait_poll_max_s"] == 2.0
