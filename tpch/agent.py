"""TPC-H benchmark entrypoint. Mirrors analytics/agent.py so the existing
...
from tpch import pipeline


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="TPC-H query benchmark (DuckDB)")
    p.add_argument("--n", type=int, default=2,
                   help="passes over the 22-query set (data size is fixed by "
                        "the fixture's scale factor)")
    p.add_argument("--seed", type=int, default=42,
                   help="accepted for harness compatibility; TPC-H is "
