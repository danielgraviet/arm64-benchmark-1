"""TPC-H query pipeline: the analytics replacement that actually uses cores.
...
line, deterministic checksum) but:
  ...
  * the timed work is DuckDB's 22 standard TPC-H queries, executed by its own
...
FIXTURE = os.environ.get("TPCH_DB", "/app/tpch/tpch.duckdb")
...
# parallelism at all.
THREADS = os.environ.get("TPCH_THREADS")
...
def run(n: int, seed: int) -> dict[str, Any]:
    ...
    if not os.path.exists(FIXTURE):
        raise RuntimeError(
            f"TPC-H fixture not found at {FIXTURE}. Build it with "
            f"build_fixtures.py (or set TPCH_DB)."
        )
    ...
    for _ in range(n):
        for q in QUERIES:
            rows = con.execute(f"PRAGMA tpch({q})").fetchall()
            rows_seen += len(rows)
